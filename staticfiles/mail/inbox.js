...
function viewAllMail(emails, boxName) {
  ...
  <span> <p> ${item.sender} <span > ${item.subject}</span> <span class="float-right">${item.timestamp} </span>   </p></span>
      ...
      if (item.read == true) {
          element.className =
              " container  p-4 m-1 bg-info  border border-black ";
          element.innerHTML += `<button class="btn btn-outline-dark m-2 " onclick="unReadMail(${item.id})"> Un Read   </button>`;
      } else {
          element.className = " container   p-4 m-1 bg-light   border border-black";
          element.innerHTML += ` <button class="btn btn-outline-dark m-2 " onclick="updatReadMail(${item.id})"> Read  </button>`;
      }
      if (boxName == "inbox") {
          element.innerHTML += `
...
function viewOneMail(emails) {
  ...
  <button class="btn btn-outline-dark " onclick="replay(${emails["id"]})">Reply </button>
  <hr>
